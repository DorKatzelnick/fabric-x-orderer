...
func TestSendConfigUpdate(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		routerState, configSeq := testSetup.routerNode.GetStatus()
		return routerState == router.RouterStateRunning && configSeq == 0
	...
	require.Eventually(t, func() bool {
		routerState, configSeq := testSetup.routerNode.GetStatus()
		return routerState == router.RouterStateRunning && configSeq == 1
	}, 20*time.Second, 100*time.Millisecond)
}
...
func TestPartyEvicted(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		routerState, configSeq := testSetup.routerNode.GetStatus()
		return routerState == router.RouterPendingAdmin && configSeq == 0
	}, 20*time.Second, 100*time.Millisecond)
}
