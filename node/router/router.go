...
func NewRouter(config *nodeconfig.RouterNodeConfig, configuration *config.Configuration, logger *flogging.FabricLogger, signer identity.SignerSerializer, configUpdateProposer policy.ConfigUpdateProposer, configRulesVerifier verify.OrdererRules) *Router {
	...
	r := createRouter(shardIDs, batcherEndpoints, tlsCAsOfBatchers, metrics, config, configuration, logger, verifier, signer, configStore, configSubmitter, decisionPuller, routerWAL)
	r.init()
	r.metrics.Start()
	return r
}
...
func (r *Router) StartRouterService() <-chan struct{} {
	...
	r.armaStopChan = make(chan struct{})
	...
	utils.StopSignalListen(r.stopChan, r, r.logger, r.Address())

	return r.armaStopChan
}
...
func (r *Router) SoftStop() error {
	configBlock, err := r.configStore.Last()
	if err != nil {
		return errors.Wrapf(err, "failed to get last config block from config store")
	}

	routerAddress := r.net.Address()
	partyID := r.routerNodeConfig.PartyID

	r.logger.Infof("Initiating soft  x stop of router listening on %s, PartyID: %d", routerAddress, partyID)
	...
	err = r.ApplyLastConfig(configBlock)
	if err != nil {
...
func (r *Router) ApplyLastConfig(configBlock *common.Block) error {
	// get last config block from config store
	// configBlock, err := r.configStore.Last()
	// if err != nil {
	// 	return errors.Wrapf(err, "failed to get last config block from config store")
	...
	requireRestart, err := config.IsNodeConfigChangeRestartRequired(currPartyConfig.RouterConfig, newPartyConfig.RouterConfig)
	if err != nil {
	...
	newRouter := NewRouter(newRouterNodeConfig, newConfiguration, r.logger, r.signer, &policy.DefaultConfigUpdateProposer{}, &verify.DefaultOrdererRules{})
	ch := newRouter.StartRouterService()
	<-ch
...
func (r *Router) pullAndProcessDecisions() {
	...
	for {
		...
		case decision, ok := <-decisionsChan:
			...
			r.logger.Warnf("Soft x stop")
			go func() {
				err := r.SoftStop()
				if err != nil {
					r.logger.Warnf("The router was not restarted properly: %v. Closing arma process..", err)
					close(r.armaStopChan)
				}
			}()
